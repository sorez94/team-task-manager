...
import { useBoardStatusPrefs } from "@/lib/useBoardStatusPrefs";
...
export function TaskBoard({ tasks, isFiltered }: { tasks: Task[]; isFiltered: boolean }) {
  ...
  const columnPrefs = useBoardStatusPrefs();
  ...
  const visibleStatuses = STATUS_OPTIONS.filter((status) => columnPrefs.visible.includes(status));
